            ...
            {updateTasksList().map((task) => {
                return (<Row className='p-2 ' key={task.id}>
                    <Col className='d-flex align-items-center justify-content-start bg-white my-1 p-1 border border-info' >
                        <Form.Check readOnly checked={task.checked ? 'checked' : ''} type='checkbox' className='ms-2' onClick={() => onChecked(task.id)} />
...
{/* <Form.Check checked={task.checked ? 'checked' : ''} type='checkbox' className='ms-2' onClick={() => onChecked(task.id)} /> */}
